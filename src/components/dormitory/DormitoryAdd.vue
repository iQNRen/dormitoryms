...
<script>
import {dormitorySave} from '../../api/dormitory'
import {buildingListNone} from '../../api/building'
    export default {
        data() {
            return {
                category:null,
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('http://localhost:8181/dormitory/save',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
        ...
        created() {
            const _this = this
            buildingListNone().then(function (resp) {
