...
<script>
import {dormitoryAdminUpdate,dormitoryAdminFindById} from '../../api/dormitoryAdmin'
    export default {
        data() {
            return {
                category:null,
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.put('http://localhost:8181/dormitoryAdmin/update',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
        ...
        created() {
            const _this = this
            this.$axios.get('http://localhost:8181/dormitoryAdmin/findById/'+_this.$route.query.id).then(function (resp) {
