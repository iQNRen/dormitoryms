...
export default {
  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          _this.ruleForm.page = _this.currentPage;
            dormitorySearch(_this.ruleForm).then(function (resp) {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value == "") {
        dormitoryLists(currentPage,_this.pageSize).then(function (resp) {
            _this.tableData = resp.data.data;
            _this.total = resp.data.total;
          });
      } else {
        _this.ruleForm.page = _this.currentPage;
        dormitorySearch(_this.pageSize).then(function (resp) {
            _this.tableData = resp.data.data;
            _this.total = resp.data.total;
          });
      }
    ...
    del(row) {
      ...
      this.$confirm("确认删除【" + row.name + "】吗？", "提示", {
        ...
        dormitoryDeleteById(row.id).then(function (resp) {
            if (resp.code == 0) {
  ...
  created() {
    const _this = this;
    dormitoryList(_this.pageSize).then(function (resp) {
        _this.tableData = resp.data.data;
        _this.total = resp.data.total;
      });
  },
