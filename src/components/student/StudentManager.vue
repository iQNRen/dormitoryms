...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          studentSearch(_this.ruleForm).then(function (resp) {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value == "") {
        studentLists(currentPage, _this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data;
          _this.total = resp.data.total;
        });
      } else {
    ...
    del(row) {
      ...
      this.$confirm("确认删除【" + row.name + "】吗？", "提示", {
        ...
        studentDeleteById(row.id).then(function (resp) {
          if (resp.code == 0) {
  ...
  created() {
    const _this = this;
    studentList(_this.pageSize).then(function (resp) {
      _this.tableData = resp.data.data;
      _this.total = resp.data.total;
    });
  },
