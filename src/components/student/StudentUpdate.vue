        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="宿舍：" prop="dormitoryId">
                <el-select v-model="ruleForm.dormitoryId" placeholder="请选择宿舍">
                    <el-option v-for="item in dormitoryList" :label="item.name" :key="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
...
<script>
import {studentUpdate,studentFindById} from '../../api/Student'
import {dormitoryAvailableList} from '../../api/dormitory'
    export default {
        data() {
            return {
                category:null,
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.put('http://localhost:8181/student/update',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
        ...
        created() {
            const _this = this
            dormitoryAvailableList().then(function (resp) {
                _this.dormitoryList = resp.data
            })
            studentFindById(_this.$route.query.id).then(function (resp) {
